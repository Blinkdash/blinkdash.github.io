class Game {
    ...
    startLevel() {
        ...
        if (!levelConfig) {
            // Если уровней больше нет, показываем окно завершения игры
            ...
            overlay.innerHTML = `
                <div class="level-complete-content">
                    ...
                    <button class="next-level-btn">Завершить игру</button>
                </div>
            `;
        ...
        // Создаем поле в зависимости от типа уровня
    ...
    createProgressIndicator() {
        ...
        let totalItems = this.currentTask.items.length;
    ...
    handleClick(item, element) {
        ...
        if (this.currentTask.type === "sequence") {
            ...
            if (isCorrect) {
                ...
                if (this.selectedItems.length === this.currentTask.items.length) {
        ...
        if (this.currentTask.type === "pairs" || this.currentTask.type === "blindPairs") {
            ...
            if (this.selectedItems.length === 0) {
                ...
                if (firstCard.item.pair === item.pair) {
                    ...
                    if (document.querySelectorAll('.game-item.correct').length === this.currentTask.items.length) {
                        setTimeout(() => this.showLevelComplete(), 500);
                    }
                } else {
                    element.classList.add('wrong');
        ...
        overlay.innerHTML = `
            <div class="level-complete-content">
                ...
                <div class="level-stats">
                    ...
                    <p>Пройдено роней: ${this.currentLevel - 1}</p>
                    <p>Сложность: ${this.gameConfig.name}</p>
                    <p>Общее время: ${totalTime} сек</p>
                </div>
    ...
    createKeyboardField() {
        ...
        const keyHandler = (e) => {
            ...
            if (isCorrect) {
                ...
                setTimeout(() => sequenceDisplay.classList.add('hidden'), 1000);
            }
        };
        ...
        }, this.currentTask.showTime * 1000);
    }
}
